...
| `28a3d227c64` | ddl, statistics: guard materialized view partition DDL (#71012) | PR2b DDL guard 补齐：partitioning / exchange partition / statistics metadata event |
...
本审计此前识别出一批 PR1-PR3 代码已在 master、但 source 的直接回归测试尚未迁移的
场景。它们已在 `pingcap/tidb#71025`（commit `7f81882ffe`）中补齐，**当前状态为 PR
待合入，不能在下文标为“master 已覆盖”**。该 PR 的 targeted tests 已在该分支验证通过。

| Source commit # | 本次补齐的 source 测试语义 | #71025 对照 |
| ---: | --- | --- |
| 7 | `ReservedRowIDAlloc.Current()` 的 base/max 断言 | `pkg/sessionctx/stmtctx/stmtctx_test.go` |
| 10 | CREATE MLog 被 base-table transaction 阻塞、提交后新事务写 MLog | `pkg/ddl/tests/metadatalock/mdl_test.go` |
| 13 | `COUNT(column)`、aggregate 大小写、缺 MLog 列的 CREATE MV validation | `pkg/executor/test/ddl/materialized_view_basic_test.go` |
| 15 | MV-dependent base table 的 `SET TIFLASH REPLICA` | `pkg/executor/test/ddl/materialized_view_basic_test.go` |
| 24 | MV physical table 的 pre-split metadata 和 regions | `pkg/executor/test/ddl/materialized_view_create_test.go` |
| 26 | `CheckMViewUpdatable` 的 direct/restricted-maintenance 路径 | `pkg/planner/core/util_test.go` |
| 38 | MLog vector index 拒绝 | `pkg/executor/test/ddl/materialized_view_alter_test.go` |
| 53 | nullable aggregate 的 CREATE MV validation | `pkg/executor/test/ddl/materialized_view_basic_test.go` |
| 63 | 五张 MV/MLog maintenance system table 的 BR unrecoverable 标记 | `br/pkg/restore/snap_client/export_test.go` |
| 71 | 有 MLog 的 base table 上 `IMPORT INTO` 拒绝 | `pkg/executor/import_into_test.go` |
| 73 | `analyzeMVColumnUsage`/related-column field type 的内部回归 | `pkg/ddl/mview_worker_test.go` |
| 82 | partition base table 上 CREATE MLog/MV 的拒绝 | `pkg/ddl/mv_index_test.go` |
| 83 | MV 上 unique index/primary key 的拒绝 | `pkg/ddl/mv_index_test.go` |
| 89 | MV/MLog base table 的 comment-only `MODIFY COLUMN` | `pkg/executor/test/ddl/materialized_view_basic_test.go` |
...
| 7 | `TestReservedRowIDAlloc` 的 `Current()` 断言，以及 MLog row-ID integration 回归。 | **当前 master 部分覆盖；#71025 补齐待合入。** master 的 MLog integration 已包含 row-ID 回归场景，`ReservedRowIDAlloc.Current()` 实现也存在；#71025 已补 source 的两个 `Current()` unit-test 断言。 |
...
| 10 | CREATE MLog `table_ids` 测试，以及 2 个 metadata-lock blocking 测试。 | **当前 master 部分覆盖；#71025 补齐待合入。** `TestCreateMaterializedViewLogJobTableIDs` 已在 master；#71025 新增两个 `TestMDLCreateMaterializedViewLog...` 并发 MDL 回归，覆盖事务阻塞和提交后新事务写 MLog。 |
...
| 13 | CREATE MV validation 回归：`CAST(0 AS UNSIGNED)` 的 build-read-TS 查询类型、`COUNT(column)`、大小写不敏感的 `SUM`/`COUNT`/`MIN`，以及缺少列时的错误信息。 | **代码覆盖 100%；#71025 补齐缺失测试待合入。** `TestCreateMaterializedViewBuildReadTSQueryTypeAlignment` 已在 master；#71025 新增 COUNT(column) 成功、大写 aggregate 成功/失败及 MLog 缺列的直接回归。 |
...
| 15 | MV 依赖 base table 执行 `SET TIFLASH REPLICA` 应放行，同时其他受 MV 约束的 DDL 仍应拒绝。 | **代码覆盖 100%；#71025 补齐 MV 专项测试待合入。** master 的 `isAlterTiFlashReplica` 特例和 MV-related DDL constraint 已实现该行为；#71025 在有 MV/MLog 依赖的 base table 上直接验证该 DDL 不会被 dependency guard 拒绝。 |
...
| 24 | CREATE MV/MLog 透传 `SHARD_ROW_ID_BITS`、`PRE_SPLIT_REGIONS`，验证 parser restore/重复 option 错误、MLog physical metadata/region 和 MV physical metadata/region。 | **代码覆盖 100%；#71025 补齐 MV physical 测试待合入。** master 的 MV/MLog AST option、executor/schema-tracker `Options` 透传、parser 重复 option/restore 测试和 MLog pre-split 测试均存在；#71025 在同一测试中补 `mv_presplit` 的 metadata 和 region 断言。 |
...
| 26 | 禁止用户对 MV/MLog 执行 INSERT/REPLACE/UPDATE/DELETE、prepared DML、LOAD DATA、IMPORT INTO；同时为内部维护 SQL 保留 restricted-session bypass。新增 `TestCheckMViewUpdatable`，并更新 writetest/integration cleanup 路径。 | **代码覆盖 100%；#71025 补齐 direct unit test 待合入。** master 的 `CheckMViewUpdatable`、planner/executor guard 和 restricted maintenance helper 均存在，`mview_log_dml` integration 覆盖用户可见 DML；#71025 增加 source 缺少的 direct/restricted-maintenance/internal-error 分支断言。 |
...
| 55 | **部分覆盖。** CREATE MLog duplicate-column validation 及 schema-tracker 对应检查已在 master；source 的 `EXCHANGE PARTITION`、`ALTER TABLE ... PARTITION BY`、`REMOVE PARTITIONING` MV/MLog 依赖拦截已由 master #71012（`28a3d227c64`）覆盖。`latestServerInfosByInstance` 的 restart de-dup 仍因 MVS 不存在而未覆盖。 | **部分覆盖。** `TestCreateMaterializedViewLogRejectsDuplicateColumns` 已迁移到 `materialized_view_basic_test.go`；#71012 的 `TestMaterializedViewPartitionDDLConstraints`、`TestMaterializedViewPartitionDependencyConstraints` 和 `TestSubscriberHandlesMaterializedViewMetadataEvents` 覆盖 partition DDL guard 与 statistics metadata no-op。server restart/DDL-ID 去重测试仍无 master 对照。 |
...
| # | Source 代码核对 | Source 测试核对 |
| ---: | --- | --- |
| 71 | **代码覆盖 100%（重构后）**。master 在 DROP/TRUNCATE 入口统一通过 `checkTableMaterializedViewConstraints` 拒绝截断 MV、MLog 及带 MLog/MV 依赖的 base table；`IMPORT INTO` builder 也拒绝带 MLog 的 base table。原 commit 在 `TruncateTable` 中直接检查 `MLogID` 的写法已被这一统一约束替代。 | **当前 master 部分覆盖；#71025 补齐待合入。** `TestDropTableMaterializedViewConstraints` 已覆盖 TRUNCATE 场景；#71025 新增 `TestImportIntoRejectsMaterializedViewLogBaseTable`，直接验证 `IMPORT INTO ... FROM SELECT WITH DISABLE_PRECHECK` 被 guard 拒绝。 |
...
| 73 | **部分覆盖**。master 已有 MLog `ADD COLUMN`、base-column 到 MLog/MV 的类型和默认值同步、tracked NULL 到 NOT NULL 拒绝、online DDL 安全性、schema tracker 和 involving-schema 处理。source 同时包含的 `MViewShadow` 可读性语义依赖 out-of-place refresh/cutover，master 没有对应 owner runtime，因此该混合 commit 不能整体记为 DDL 已覆盖。 | **主体覆盖；#71025 补齐 4 个内部 DDL 单测待合入。** #71025 新增 alias/ordinal GROUP BY、WHERE 引用拒绝和 related-column field type flags 覆盖。`TestCheckMViewShadowReadable` 仍依赖未 port 的 shadow/cutover runtime。 |
...
| 89 | **代码覆盖 100%**。master `pkg/ddl/modify_column.go` 的 `isColumnCommentOnlyChange` 会在 clone 后仅忽略 comment 字段比较；base table 存在 MV/MLog 依赖时，comment-only MODIFY 跳过类型/依赖约束和相关表同步，实际类型变化仍被拒绝。 | **#71025 补齐 MV 专项回归待合入。** 新增 comment-only SQL 和 `information_schema.columns.column_comment` 断言，并在 base table 已有 MV/MLog 依赖时执行。 |
...
| 7 | `b476f8f72604` | 2026-02-13 | `table, sessionctx: fix bug where mlog consumes reserved row IDs from base table (#66246)` | 修正 MLog 写入对 base table 保留 row ID/handle 的影响。 | PR3 | **代码覆盖；测试由 #71025 补齐待合入。** master `pkg/table/tables/mview_log.go` 在写 MLog 前暂时清空 `ReservedRowIDAlloc`、写入后恢复 base-table allocation，MLog row-ID integration 已存在；#71025 补两个 `Current()` 断言。 |
...
| 10 | `f7f7e7f85cda` | 2026-02-15 | `pkg/ddl: include base table id for create mlog MDL table ids (#66266)` | CREATE MLog 的 metadata lock involving tables 加入 base table，保证相关 DDL 串行化。 | PR2b-create | **代码覆盖；测试由 #71025 补齐待合入。** master `pkg/ddl/jobsubmit/submit.go` 的 `job2TableIDs` 对 `ActionCreateMaterializedViewLog` 将 job table ID 与 `MaterializedViewLog.BaseTableID` 一并写入 `table_ids`；#71025 增加 base-table transaction 阻塞 CREATE MLog 和提交后写 MLog 回归。 |
...
| 13 | `76ef4adbc110` | 2026-02-21 | `ddl, executor: fix CREATE MATERIALIZED VIEW validation (#66316)` | 修正 CREATE MV 的对象类型、查询和依赖 validation。 | PR2b-create | **代码覆盖；测试由 #71025 补齐待合入。** master #70789 的 validation 和 build-read-TS 类型测试已覆盖基础语义；#71025 补 COUNT(column)、aggregate 大小写和 MLog 缺列的直接回归。 |
...
| 15 | `0f22124d57d0` | 2026-02-24 | `ddl: allow SET TIFLASH REPLICA on MV-dependent base tables (#66328)` | 放宽 MV 相关表的 TiFlash replica placement DDL，同时保留其他 base-table DDL 约束。 | PR2b-create/drop/alter | **代码覆盖；MV 专项测试由 #71025 补齐待合入。** master 的 MV-related DDL constraint 已实现该特例；#71025 直接验证 MV/MLog 依赖不触发 dependency guard。 |
...
| 26 | `3b8b6e4a7230` | 2026-02-28 | `planner, executor, sessionctx: reject explicit dml on mview / mlog tables (#66396)` | 拦截用户直接对 MV/MLog 的 DML，并覆盖 planner fast path 和 integration test。 | PR3 | **代码覆盖；direct unit test 由 #71025 补齐待合入。** master #70941 的 planner/executor guard 和 integration 覆盖用户可见 DML；#71025 补 `TestCheckMViewUpdatable` 的 maintenance/restricted SQL 分支。 |
...
| 38 | `adc727df5a8b` | 2026-05-28 | `*: several ddl enhancement for mv (#68649)` | 大量 DDL worker、job args、schema tracker、multi-schema-change、依赖约束和测试增强。 | PR2b-create/drop/alter | **代码 100%（重构后），测试部分覆盖。** #71025 补 MLog vector-index 专项回归待合入；旧的 refresh/purge metadata 测试被 Unix-seconds/权限测试重构，`TestPurgeMaterializedViewLogDisallowExplicitTransaction` 及其 purge runtime 测试仍待 PR4。 |
...
| 53 | `bdb304c4d378` | 2026-06-03 | `ddl, executor, planner: backport duplicate count expr support for nullable MV aggs (#68893)` | 修正 nullable MV aggregate 的 duplicate count expression，并更新 planner/executor/DDL 测试。 | PR2b-create validation；PR5 S9/S10 | **部分覆盖。** #71025 补 nullable SUM/COUNT、duplicate COUNT 和 nullable MIN/MAX 的 CREATE validation 回归待合入；`ArgNotNull`、duplicate-count delta-merge planner/executor 和 fast-refresh runtime 测试仍待 PR5。 |
...
| 55 | `7a3b5d9443e1` | 2026-06-04 | `ddl, mvservice: backport materialized view maintenance fixes (#68947)` | 混合 DDL worker 和 MV service maintenance 修复。 | PR2b；PR7 | **部分覆盖。** CREATE MLog duplicate-column validation/schema tracker 已覆盖；source 的 EXCHANGE PARTITION、ALTER/REMOVE PARTITIONING MV/MLog guards 及其 executor/DDL helper/statistics metadata-event 回归已由 master #71012 覆盖。MVS server restart de-dup 及其测试仍未覆盖。 |
...
| 71 | `78e1033f4aa7` | 2026-06-16 | `ddl, executor: reject truncating mview-related tables with mlog guards (#69218)` | 拒绝会破坏 MV/MLog 依赖的 TRUNCATE/相关 DDL，并增加 worker 侧约束。 | PR2b-drop/alter | **代码覆盖；测试由 #71025 补齐待合入。** master 的统一 `checkTableMaterializedViewConstraints` 和 IMPORT base-table guard 已覆盖最终代码语义；#71025 新增 `TestImportIntoRejectsMaterializedViewLogBaseTable`。 |
...
| 82 | `e7a30c7889ba` | 2026-06-19 | `ddl: forbid the creating of mlog on partition base table (#68903)` | 拒绝对 partitioned base table 创建 MLog，并让 CREATE MV 分区表错误正确取消。 | PR2b-create | **代码覆盖；测试由 #71025 补齐待合入。** MLog/CREATE MV 的 executor、worker、schema tracker 检查已在 master；#71025 直接覆盖 CREATE MLog 和 CREATE MV 两条分区表拒绝路径。 |
| 83 | `4769ef44fab9` | 2026-06-20 | `ddl: forbid unique indexes on materialized view (#68911)` | 拒绝会破坏 MV 维护语义的 unique index 操作。 | PR2b-alter | **代码覆盖；专项测试由 #71025 补齐待合入。** unique/primary key 的 MV 约束已在 master；#71025 覆盖 CREATE UNIQUE INDEX、ALTER ADD UNIQUE/PRIMARY KEY 拒绝和普通 index 允许。 |
...
| 89 | `0fd4796db0c4` | 2026-06-23 | `ddl: allow comment-only modify on materialized view base columns (#69391)` | 允许不改变数据语义的 comment-only base column 修改，同时保持 MLog 约束。 | PR2b-alter | **代码覆盖；MV 专项回归由 #71025 补齐待合入。** comment-only helper 和约束例外已在 master；#71025 在 MV/MLog 依赖下覆盖 SQL 和 `column_comment` 断言。 |
