...
func (e *RecoverIndexExec) batchMarkDup(txn kv.Transaction, rows []recoverRows) error {
	...
	for i, row := range rows {
		idxKey, distinct, err := e.index.GenIndexKey(sc, row.idxVals, row.handle, e.idxKeyBufs[i], nil)
		if err != nil {
			return err
		}
...
func (e *CleanupIndexExec) fetchIndex(ctx context.Context, txn kv.Transaction) error {
	...
	for {
		...
		for row := iter.Begin(); row != iter.End(); row = iter.Next() {
			...
			idxKey, _, err := e.index.GenIndexKey(sc, idxVals, handle, nil, nil)
			if err != nil {
				return err
			}
			e.scanRowCnt++
...
func (e *CleanupIndexExec) init() error {
	...
	e.idxValsBufs = make([][]types.Datum, e.batchSize)
	sc := e.ctx.GetSessionVars().StmtCtx
	idxKey, _, err := e.index.GenIndexKey(sc, []types.Datum{{}}, kv.IntHandle(math.MinInt64), nil, nil)
	if err != nil {
		return err
	}
	e.lastIdxKey = idxKey
