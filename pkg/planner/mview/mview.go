...
func FindVisibleIndexesWithPrefixCoveringColumns(
	...
	names := make([]ast.CIStr, 0, len(indexNames))
	for _, indexName := range indexNames {
		names = append(names, ast.NewCIStr(indexName))
	}
	return names
}

// SetFullUpdateLookupIndexHint binds the full-update lookup inner scan to the supporting indexes.
func SetFullUpdateLookupIndexHint(sel *ast.SelectStmt, indexNames []ast.CIStr) error {
	...
	baseTableName.IndexHints = []*ast.IndexHint{
		{
			IndexNames: append([]ast.CIStr(nil), indexNames...),
...
func buildMLogDeltaSelect(
	...
	phase1Fields = append(phase1Fields, &ast.SelectField{
		...
		AsName: ast.NewCIStr(deltaCntStarName),
	})
	...
	for _, ac := range aggCols {
		...
		case AggCount:
			...
			phase1Fields = append(phase1Fields, &ast.SelectField{
				...
				AsName: ast.NewCIStr(ac.deltaName),
			})
		case AggSum:
			if ac.argExpr == nil {
		...
		case AggMax:
			...
			phase1Fields = append(phase1Fields,
				...
				&ast.SelectField{
					...
					AsName: ast.NewCIStr(ac.addedCountDeltaName),
				},
				&ast.SelectField{
					Expr:   aggMax(removedArg),
					AsName: ast.NewCIStr(ac.removedValueDelta),
				},
				&ast.SelectField{
					Expr:   aggMaxCount(removedArg),
					AsName: ast.NewCIStr(ac.removedCountDelta),
				},
			)
		case AggMin:
			if ac.info.ArgColName == "" {
	...
	tableHints := []*ast.TableOptimizerHint{
		{
			HintName: ast.NewCIStr(utilhint.HintReadFromStorage),
			HintData: ast.NewCIStr(utilhint.HintTiFlash),
...
func buildMergeSourceSelect(
	...
	deltaSrc := &ast.TableSource{Source: deltaSel, AsName: ast.NewCIStr(deltaTableAlias)}
	mvSrc := &ast.TableSource{
		Source: &ast.TableName{Schema: dbName, Name: mv.Name},
		AsName: ast.NewCIStr(mvTableAlias),
	}
	...
	if !mv.PKIsHandle && !mv.IsCommonHandle {
		...
		fields = append(fields, &ast.SelectField{
			...
			AsName: ast.NewCIStr(mvRowIDName),
		})
	}
...
func buildFullUpdateLookupTemplateSelect(
	...
	outerSrc := &ast.TableSource{Source: outerSel, AsName: ast.NewCIStr(fullUpdateOuterAlias)}
	innerSrc := &ast.TableSource{Source: innerSel, AsName: ast.NewCIStr(fullUpdateInnerAlias)}
		...
		TableHints: []*ast.TableOptimizerHint{
			{
				...
				Tables: []ast.HintTable{
					{DBName: dbName, TableName: ast.NewCIStr(fullUpdateOuterAlias)},
					{DBName: dbName, TableName: ast.NewCIStr(fullUpdateInnerAlias)},
				},
			},
		},
	}, mvOffsets, nil
...
func buildFullUpdateLookupInnerSelect(
	...
	for i, mvCol := range mvCols {
		if _, ok := groupKeySet[i]; ok {
			...
			fields = append(fields, &ast.SelectField{
				Expr:   baseColExpr,
				AsName: ast.NewCIStr(groupKeyBaseColByMVOffset[i]),
			})
			continue
		}
...
func dbNameByTableID(is infoschema.InfoSchema, tableID int64) (ast.CIStr, error) {
	item, ok := is.TableItemByID(tableID)
	if !ok {
		return ast.CIStr{}, errors.Errorf("table id %d not found in infoschema", tableID)
	}
	if item.DBName.L == "" {
		return ast.CIStr{}, errors.Errorf("table id %d has empty schema name in infoschema", tableID)
	}
...
func (*columnQualifierStripper) Enter(n ast.Node) (ast.Node, bool) {
	...
	cn.Name.Schema = ast.CIStr{}
	cn.Name.Table = ast.CIStr{}
	return n, false
}
...
func qualColExpr(tableAlias string, colName string) *ast.ColumnNameExpr {
	return &ast.ColumnNameExpr{Name: &ast.ColumnName{
		Table: ast.NewCIStr(tableAlias),
		Name:  ast.NewCIStr(colName),
	}}
}

func colExpr(colName string) *ast.ColumnNameExpr {
	return &ast.ColumnNameExpr{Name: &ast.ColumnName{Name: ast.NewCIStr(colName)}}
}
...
func buildReadFromStorageTiFlashHint(dbName, tableName ast.CIStr) *ast.TableOptimizerHint {
...
func buildHashJoinProbeHint(dbName, tableName ast.CIStr) *ast.TableOptimizerHint {
	return &ast.TableOptimizerHint{
		HintName: ast.NewCIStr(utilhint.HintHashJoinProbe),
		Tables: []ast.HintTable{
			{
				DBName:    dbName,
				TableName: tableName,
...
func BuildCompleteDiffSource(
	...
	recomputedSource := &ast.TableSource{
		Source: mvSel,
		AsName: ast.NewCIStr(diffRecomputedAlias),
	}
	currentSource := &ast.TableSource{
		Source: &ast.TableName{Schema: dbName, Name: mv.Name},
		AsName: ast.NewCIStr(diffCurrentAlias),
	}
	...
	fields = append(fields, &ast.SelectField{Expr: diffOpExpr, AsName: ast.NewCIStr(diffOpColName)})
	...
	if useRowID {
		...
		fields = append(fields, &ast.SelectField{
			...
			AsName: ast.NewCIStr(diffHandlePrefix + model.ExtraHandleName.O),
		})
	}
	...
	for i, col := range mv.Columns {
		...
		fields = append(fields, &ast.SelectField{
			...
			AsName: ast.NewCIStr(diffOldRowPrefix + col.Name.O),
		})
	...
	for i, col := range mv.Columns {
		...
		fields = append(fields, &ast.SelectField{
			...
			AsName: ast.NewCIStr(diffNewRowPrefix + col.Name.O),
		})
	...
	diffSel := &ast.SelectStmt{
		...
		Where:  whereExpr,
		TableHints: []*ast.TableOptimizerHint{
			buildReadFromStorageTiFlashHint(dbName, ast.NewCIStr(diffCurrentAlias)),
			buildHashJoinProbeHint(dbName, ast.NewCIStr(diffCurrentAlias)),
		},
	}
