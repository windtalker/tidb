...
func TestMaterializedViewDDLBasic(t *testing.T) {
	...
	require.False(t, mvTable.Meta().MaterializedView.MViewRefreshFailed)
	...
	require.False(t, mvAlertTable.Meta().MaterializedView.MViewRefreshFailed)
	...
	require.False(t, mvAlertZeroTable.Meta().MaterializedView.MViewRefreshFailed)
	tk.MustExec("create materialized view mv_alert_failed (a, cnt) refresh fast attributes='mview_refresh_failed=yes' as select a, count(1) from t group by a")
	is = dom.InfoSchema()
	mvAlertFailedTable, err := is.TableByName(context.Background(), pmodel.NewCIStr("test"), pmodel.NewCIStr("mv_alert_failed"))
	require.NoError(t, err)
	require.True(t, mvAlertFailedTable.Meta().MaterializedView.MViewRefreshFailed)
	...
	err = tk.ExecToErr("create materialized view mv_alert_bad_failed (a, cnt) attributes='mview_refresh_failed=maybe' as select a, count(1) from t group by a")
...
func TestShowCreateMaterializedView(t *testing.T) {
	...
	tk.MustExec("create materialized view mv_show_mv_attr (a, s, cnt) refresh fast attributes='mview_alert_warning=5,mview_alert_overdue=10,mview_refresh_failed=yes' as select a, sum(b), count(1) from t_show_mv group by a")
	...
	require.True(t, ok)
	require.Contains(t, attrShowCreate, "ATTRIBUTES='mview_alert_warning=5,mview_alert_overdue=10,mview_refresh_failed=yes'")
...
func TestAlterMaterializedViewAttributesUpdatesAlertThresholds(t *testing.T) {
	...
	tk.MustExec("insert into t values (1, 10), (1, 5), (2, 7)")
	tk.MustExec("create materialized view log on t (a, b) purge next date_add(now(), interval 1 hour)")
	tk.MustExec("create materialized view mv_attr (a, s, cnt) refresh fast next date_add(now(), interval 2 hour) attributes='mview_alert_warning=300,mview_alert_overdue=600,mview_refresh_failed=no' as select a, sum(b), count(1) from t group by a")
	...
	require.False(t, mvInfo.MViewRefreshFailed)
	require.Equal(t, "DATE_ADD(NOW(), INTERVAL 2 HOUR)", mvInfo.RefreshNext)

	tk.MustExec("alter materialized view mv_attr attributes='mview_alert_warning=5,mview_alert_overdue=5,mview_refresh_failed=yes'")
	mvInfo = getMViewMeta()
	require.Equal(t, int64(5), mvInfo.AlertWarningSec)
	require.Equal(t, int64(5), mvInfo.AlertOverdueSec)
	require.True(t, mvInfo.MViewRefreshFailed)
	require.Equal(t, "DATE_ADD(NOW(), INTERVAL 2 HOUR)", mvInfo.RefreshNext)
	...
	tk.MustExec("alter materialized view mv_attr attributes='mview_alert_warning=10,mview_alert_overdue=20,mview_refresh_failed=no'")
	mvInfo = getMViewMeta()
	...
	require.ErrorContains(t, err, "unsupported ATTRIBUTES key")
	err = tk.ExecToErr("alter materialized view mv_attr attributes='mview_refresh_failed=maybe'")
	require.ErrorContains(t, err, "must be yes or no")
