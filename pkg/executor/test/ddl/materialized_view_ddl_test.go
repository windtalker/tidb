...
func TestMaterializedViewDDLBasic(t *testing.T) {
	...
	tk.MustExec("create table t_minmax_group_expr_ok (a int not null, b datetime not null, c int not null, index idx_date_a((date(b)), a))")
	tk.MustExec("create materialized view log on t_minmax_group_expr_ok (a, b, c) purge next date_add(now(), interval 1 hour)")
	tk.MustExec("create materialized view mv_minmax_group_expr_ok (a, d, minc, c1) as select a, date(b), min(c), count(1) from t_minmax_group_expr_ok group by a, date(b)")
	tk.MustExec("create table t_minmax_group_expr_bad (a int not null, b datetime not null, c int not null, index idx_a_year(a, (year(b))))")
	tk.MustExec("create materialized view log on t_minmax_group_expr_bad (a, b, c) purge next date_add(now(), interval 1 hour)")
	err = tk.ExecToErr("create materialized view mv_minmax_group_expr_bad (a, d, minc, c1) as select a, date(b), min(c), count(1) from t_minmax_group_expr_bad group by a, date(b)")
	require.ErrorContains(t, err, "requires base table index whose leading columns cover all GROUP BY columns")
	...
	tk.MustExec("drop materialized view mv_minmax_group_expr_ok")
	...
	tk.MustExec("drop materialized view log on t_minmax_group_expr_ok")
	tk.MustExec("drop materialized view log on t_minmax_group_expr_bad")
