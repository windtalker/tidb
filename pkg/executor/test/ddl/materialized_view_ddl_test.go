...
func TestMaterializedViewDDLBasic(t *testing.T) {
	...
	is = dom.InfoSchema()
	mvCountColTable, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv_count_col"))
	require.NoError(t, err)
	...
	is = dom.InfoSchema()
	mvAlertTable, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv_alert"))
	require.NoError(t, err)
	...
	is = dom.InfoSchema()
	mvAlertZeroTable, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv_alert_zero"))
	require.NoError(t, err)
	...
	is = dom.InfoSchema()
	mvAlertFailedTable, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv_alert_failed"))
	require.NoError(t, err)
	...
	is = dom.InfoSchema()
	mvPreSplit, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv_presplit"))
	require.NoError(t, err)
...
func TestAlterMaterializedViewAttributesUpdatesAlertThresholds(t *testing.T) {
	...
	getMViewMeta := func() *model.MaterializedViewInfo {
		is := dom.InfoSchema()
		mvTable, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv_attr"))
		require.NoError(t, err)
		require.NotNil(t, mvTable.Meta().MaterializedView)
...
func TestMaterializedViewDDLLifecycle(t *testing.T) {
	store, dom := testkit.CreateMockStoreAndDomain(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("create table t (a int not null, b int not null)")

	err := tk.ExecToErr("create materialized view mv_without_log (a, cnt) as select a, count(1) from t group by a")
	require.ErrorContains(t, err, "materialized view log does not exist")
	...
	require.Equal(t, []ast.CIStr{ast.NewCIStr("a"), ast.NewCIStr("b")}, mlogTable.Meta().MaterializedViewLog.Columns)
	...
	require.Equal(t, "FAST", mvTable.Meta().MaterializedView.RefreshMethod)
	require.Equal(t, "DATE_ADD(NOW(), INTERVAL 1 HOUR)", mvTable.Meta().MaterializedView.RefreshNext)
	tk.MustQuery(fmt.Sprintf("select LAST_SUCCESS_READ_TSO > 0 from mysql.tidb_mview_refresh_info where MVIEW_ID = %d", mvTable.Meta().ID)).Check(testkit.Rows("1"))

	require.NotNil(t, baseTable.Meta().MaterializedViewBase)
	require.Equal(t, mlogTable.Meta().ID, baseTable.Meta().MaterializedViewBase.MLogID)
	require.Equal(t, []int64{mvTable.Meta().ID}, baseTable.Meta().MaterializedViewBase.MViewIDs)

	err = tk.ExecToErr("drop materialized view log on t")
	require.ErrorContains(t, err, "dependent materialized views exist")

	tk.MustExec("drop materialized view mv")
	tk.MustQuery(fmt.Sprintf("select count(*) from mysql.tidb_mview_refresh_info where MVIEW_ID = %d", mvTable.Meta().ID)).Check(testkit.Rows("0"))
	is = dom.InfoSchema()
	_, err = is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("mv"))
	require.Error(t, err)
	baseTable, err = is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("t"))
	require.NoError(t, err)
	require.NotNil(t, baseTable.Meta().MaterializedViewBase)
	...
	tk.MustExec("drop materialized view log on t")
	tk.MustQuery(fmt.Sprintf("select count(*) from mysql.tidb_mlog_purge_info where MLOG_ID = %d", mlogTable.Meta().ID)).Check(testkit.Rows("0"))
	is = dom.InfoSchema()
	baseTable, err = is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("t"))
	require.NoError(t, err)
...
func TestAlterMaterializedViewLogDDL(t *testing.T) {
	...
	require.Equal(t, []string{"a", "b", "c", "_MLOG$_DML_TYPE", "_MLOG$_OLD_NEW"}, columnNames(mlogTable.Columns))
	...
	_, mlogInfo = getMLog()
	require.Equal(t, "", mlogInfo.PurgeStartWith)
	require.Equal(t, "", mlogInfo.PurgeNext)
