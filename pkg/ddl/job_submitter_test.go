...
func TestCreateMaterializedViewJobTableIDsMultiMLog(t *testing.T) {
	...
	t.Run("refresh materialized view complete out-of-place cutover", func(t *testing.T) {
		const (
			oldMViewID   int64  = 900000000000000020
			shadowTblID  int64  = 900000000000000022
			buildReadTSO uint64 = 12345
		)
		cutoverJob := ddl.NewJobWrapperWithArgs(
			&model.Job{
				Version:    model.GetJobVerInUse(),
		...
		submitter := ddl.NewJobSubmitterForTest()
		require.NoError(t, submitter.GenGIDAndInsertJobsWithRetry(ctx, sess.NewSession(tk.Session()), []*ddl.JobWrapper{cutoverJob}))

		rows := tk.MustQuery(fmt.Sprintf("select table_ids from mysql.tidb_ddl_job where job_id = %d", cutoverJob.ID)).Rows()
		require.Len(t, rows, 1)
		tableIDs := strings.Split(rows[0][0].(string), ",")
		require.Len(t, tableIDs, 2)
