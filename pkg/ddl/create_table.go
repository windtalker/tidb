...
func (w *worker) onCreateMaterializedView(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	baseSchemaIDs := mvTblInfo.MaterializedView.BaseSchemaIDs
	if len(baseSchemaIDs) != len(baseTableIDs) {
		job.State = model.JobStateCancelled
		return ver, dbterror.ErrInvalidDDLJob.GenWithStackByArgs("create materialized view: base schema/table metadata length mismatch")
	}
	for _, baseSchemaID := range baseSchemaIDs {
		if baseSchemaID == 0 {
			job.State = model.JobStateCancelled
			return ver, dbterror.ErrInvalidDDLJob.GenWithStackByArgs("create materialized view: invalid base schema id")
		}
	}
	...
	case model.StateNone:
		...
		for i, baseTableID := range baseTableIDs {
			baseSchemaID := baseSchemaIDs[i]
			baseSchemaName := job.SchemaName
			if is != nil {
				if dbInfo, ok := is.SchemaByID(baseSchemaID); ok {
					baseSchemaName = dbInfo.Name.O
				}
			}
			if _, err := onCreateMaterializedViewBaseCheck(jobCtx.metaMut, baseSchemaID, baseTableID, baseSchemaName); err != nil {
				if infoschema.ErrDatabaseNotExists.Equal(err) || infoschema.ErrTableNotExists.Equal(err) {
					job.State = model.JobStateCancelled
				}
		...
		extras, err := updateMaterializedViewBaseInfoOnCreate(jobCtx, job, mvTblInfo, baseSchemaIDs)
		if err != nil {
			return ver, errors.Trace(err)
		}
	...
	case model.StateWriteReorganization:
		...
		for i, baseTableID := range baseTableIDs {
			baseTblInfo, getErr := getTableInfo(jobCtx.metaMut, baseTableID, baseSchemaIDs[i])
			if getErr != nil {
...
func updateMaterializedViewBaseInfoOnCreate(jobCtx *jobContext, job *model.Job, createdTable *model.TableInfo, baseSchemaIDs []int64) ([]schemaIDAndTableInfo, error) {
	...
	for i, baseTableID := range baseTableIDs {
		...
		schemaHint := job.SchemaID
		if i < len(baseSchemaIDs) && baseSchemaIDs[i] != 0 {
			schemaHint = baseSchemaIDs[i]
		}
		baseSchemaID := resolveSchemaIDByTableID(jobCtx, baseTableID, schemaHint)
		...
		if err := updateTable(jobCtx.metaMut, baseSchemaID, baseTblInfo); err != nil {
			job.State = model.JobStateCancelled
			return nil, errors.Trace(err)
		}
