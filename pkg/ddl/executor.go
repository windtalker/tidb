...
func checkProtectedMaterializedViewShadowConstraint(sv *variable.SessionVars, tblInfo *model.TableInfo, op string) error {
	if tblInfo.MaterializedViewShadow == nil || allowInternalMViewShadowBypass(sv) {
		return nil
	}
...
func (e *executor) dropTableObject(
	...
	for _, tn := range objects {
		...
		case tableObject:
			...
			allowMVRelated := len(allowMaterializedViewRelated) > 0 && allowMaterializedViewRelated[0]
			if !allowMVRelated {
...
func checkTableMaterializedViewConstraintsWithOptions(
	...
	if tblInfo.MaterializedViewShadow != nil && !(sv != nil && sv.InMaterializedViewMaintenance && sv.InRestrictedSQL) {
		return dbterror.ErrGeneralUnsupportedDDL.GenWithStackByArgs(fmt.Sprintf("%s on materialized view shadow table", op))
	}
	if tblInfo.MaterializedViewBase != nil && len(tblInfo.MaterializedViewBase.MViewIDs) > 0 {
